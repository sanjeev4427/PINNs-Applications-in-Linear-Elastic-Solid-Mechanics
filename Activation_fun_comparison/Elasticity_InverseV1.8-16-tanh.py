...
savetxt('Epochs {}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), np.array(epch), delimiter=',')
...
for i,N in enumerate(Nl):
    ...
    m.save_weights('{}X{}epoch{}model-elu.h5'.format(N-1,N-1,epochs))
...
savetxt('Train_time {}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(train_t2), delimiter=',')  
#saving pinn output
savetxt('Uxy_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Uxy.eval([x_test, y_test])), delimiter=',') 
savetxt('Vxy_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Vxy.eval([x_test, y_test])), delimiter=',') 
savetxt('Sxx_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Sxx.eval([x_test, y_test])), delimiter=',') 
savetxt('Syy_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Syy.eval([x_test, y_test])), delimiter=',')
savetxt('Sxy_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Sxy.eval([x_test, y_test])), delimiter=',')
savetxt('Exx_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Exx.eval([x_test, y_test])), delimiter=',')
savetxt('Eyy_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Eyy.eval([x_test, y_test])), delimiter=',')
savetxt('Exy_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Exy.eval([x_test, y_test])), delimiter=',')
...
savetxt('lambda_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(lmbd_final)[0], delimiter=',')
savetxt('mu_{}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(mu_final)[0], delimiter=',')
...
for i in range(l):
    ...
    plt.savefig('loss {}X{} grid-elu.png'.format(Nl[i]-1,Nl[i]-1))
    loss[i] = (ht[i].history['loss']);
print(loss)
#saving loss
savetxt('loss {}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(loss), delimiter=',')
...
savetxt('L2 Error_norm_disp {}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(uv_err_l2_norm), delimiter=',')
#saving L2 error norm of stresses
savetxt('L2 Error_norm_Stress {}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(S_err_l2_norm), delimiter=',')
...
savetxt('Energy Error_norm {}X{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), uv_err_energy_norm, delimiter=',')
print(type(uv_err_l2_norm))
...
savetxt('Convergence ratio in uv {}-{}-elu.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(uv_conv_ratio), delimiter=',')
...
for index,N in enumerate(Nl):
    ...
    for i,(key, val) in enumerate(test_data[index].items()):
        if i<4:
            ...
            j=j+1
    plt.tight_layout()
    plt.savefig('test output {}X{} grid-elu.png'.format(N-1,N-1))
...
for index,N in enumerate(Nl):
    ...
    plt.tight_layout()
    plt.savefig('abs error {}X{} grid-elu.png'.format(N-1,N-1))
    #plt.show()
...
for index,N in enumerate(Nl):
    ...
    plt.tight_layout()
    plt.savefig('l2 error {}X{} grid-elu.png'.format(N-1,N-1))
    #plt.show()
...
for index,N in enumerate(Nl):
    ...
    plt.tight_layout()
    plt.savefig('energy error {}X{} grid-elu.png'.format(N-1,N-1))
