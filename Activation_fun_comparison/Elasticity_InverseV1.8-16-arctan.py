...
savetxt('Epochs {}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), np.array(epch), delimiter=',')
...
#creating zero array of same size as x and y arrays
...
for i,N in enumerate(Nl):
    ...
    Uxy = sn.Functional("Uxy", [x, y], 4*[40], '{}'.format(act_f[1]))
    Vxy = sn.Functional("Vxy", [x, y], 4*[40], '{}'.format(act_f[1])) 
    ...
    ht[i] = m.train([x_data, y_data], [Lx_data, Ly_data, u_data, v_data], #syy_data], 
                ...
                adaptive_weights={'method':'NTK', 'freq': 100})#,save_weights={'save_weights_to':".PINN\Weights\{}X{}model".format(N-1,N-1), 'freq': epochs})
    m.save_weights('{}X{}epoch{}model-arctan.h5'.format(N-1,N-1,epochs))
...
savetxt('Train_time {}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(train_t2), delimiter=',')  
#saving pinn output
savetxt('Uxy_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Uxy.eval([x_test, y_test])), delimiter=',') 
savetxt('Vxy_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Vxy.eval([x_test, y_test])), delimiter=',') 
savetxt('Sxx_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Sxx.eval([x_test, y_test])), delimiter=',') 
savetxt('Syy_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Syy.eval([x_test, y_test])), delimiter=',')
savetxt('Sxy_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Sxy.eval([x_test, y_test])), delimiter=',')
savetxt('Exx_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Exx.eval([x_test, y_test])), delimiter=',')
savetxt('Eyy_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Eyy.eval([x_test, y_test])), delimiter=',')
savetxt('Exy_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(Exy.eval([x_test, y_test])), delimiter=',')
...
savetxt('lambda_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(lmbd_final)[0], delimiter=',')
savetxt('mu_{}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(mu_final)[0], delimiter=',')
...
for i in range(l):
    ...
    plt.savefig('loss {}X{} grid-arctan.png'.format(Nl[i]-1,Nl[i]-1))
    loss[i] = (ht[i].history['loss']);
print(loss)
#saving loss
savetxt('loss {}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(loss), delimiter=',')
...
#saving L2 error norm of uv displacement
savetxt('L2 Error_norm_disp {}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(uv_err_l2_norm), delimiter=',')
#saving L2 error norm of stresses
savetxt('L2 Error_norm_Stress {}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(S_err_l2_norm), delimiter=',')
# savetxt('L2 Error_norm_Syy.csv', asarray(Syy_err_l2_norm), delimiter=',')
# savetxt('L2 Error_norm_Sxy.csv', asarray(Sxy_err_l2_norm), delimiter=',')
#saving energy error norm of displacements
savetxt('Energy Error_norm {}X{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), uv_err_energy_norm, delimiter=',')
print(type(uv_err_l2_norm))
...
for index,N in enumerate(Nl):
    ...
    if (uv_act_i+1)%Nt!=0:
        ...
        uv_act_max_col = Nt-1
    uv_test_max = np.sqrt(test_data[index][r'${u_x}$'][uv_act_max_row][uv_act_max_col]**2+test_data[index][r'${u_y}$'][uv_act_max_row][uv_act_max_col]**2)
    uv_conv_ratio.append(np.abs(uv_test_max-uv_act_max)/uv_act_max)
print("Convergence ratio in uv: {}".format(uv_conv_ratio))
savetxt('Convergence ratio in uv {}-{}-arctan.csv'.format(Nl[0]-1,Nl[-1]-1), asarray(uv_conv_ratio), delimiter=',')
...
for index,N in enumerate(Nl):
    ...
    for i,(key, val) in enumerate(test_data[index].items()):
        if i<4:
            ...
            j=j+1
    plt.tight_layout()
    plt.savefig('test output {}X{} grid-arctan.png'.format(N-1,N-1))
...
for index,N in enumerate(Nl):
    ...
    plt.tight_layout()
    plt.savefig('abs error {}X{} grid-arctan.png'.format(N-1,N-1))
    #plt.show()
...
for index,N in enumerate(Nl):
    ...
    plt.tight_layout()
    plt.savefig('l2 error {}X{} grid-arctan.png'.format(N-1,N-1))
    #plt.show()
...
for index,N in enumerate(Nl):
    ...
    plt.tight_layout()
    plt.savefig('energy error {}X{} grid-arctan.png'.format(N-1,N-1))
